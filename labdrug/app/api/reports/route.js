...
export async function GET(request) {
  ...
  try {
    ...
    const [salesSummary, topProducts, salesByDay, inventorySummary, stockOuts] = await Promise.all([
      sql`SELECT COUNT(*) as transaction_count, COALESCE(SUM(total_amount),0) as total_revenue, COALESCE(AVG(total_amount),0) as avg_transaction FROM sale WHERE sale_date BETWEEN ${dateFrom} AND ${dateTo}`,
      sql`
        SELECT p.product_name, SUM(sd.quantity) as qty_sold, SUM(sd.subtotal) as revenue
      ...
      `,
      sql`SELECT sale_date, COUNT(*) as count, SUM(total_amount) as total FROM sale WHERE sale_date BETWEEN ${dateFrom} AND ${dateTo} GROUP BY sale_date ORDER BY sale_date`,
      sql`
        ...
        FROM inventory_view
      `,
      sql`SELECT COUNT(*) as count, COALESCE(SUM(total),0) as total_loss FROM stock_out WHERE stock_out_date BETWEEN ${dateFrom} AND ${dateTo}`,
    ])
