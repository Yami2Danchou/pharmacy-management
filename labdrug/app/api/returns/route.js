  ...
  try {
    const returns = await sql`
      SELECT r.return_product_id, r.return_date, r.return_type, r.reason,
        r.customer_name, r.sale_id,
        u.username, e.employee_name as verified_by,
        COUNT(rd.return_details_id) as item_count
      ...
      ORDER BY r.return_product_id DESC
      LIMIT 100
    `
...
export async function POST(request) {
  ...
  try {
    ...
    const returnResult = await sql`
      INSERT INTO return_of_products
        (return_date, return_type, reason, verified_by_employee_id, sale_id, customer_name, user_id)
      VALUES
        (CURRENT_DATE, ${return_type}, ${reason}, ${verified_by_employee_id}, ${sale_id || null}, ${customer_name || null}, ${session.userId})
    ...
    for (const item of items) {
      ...
      // Add stock back if return type is 'Resellable'
      if (return_type === 'Resellable') {
        const batch = await sql`
          SELECT batch_code FROM product_expirationdate
          WHERE product_id = ${item.product_id}
          ORDER BY expirationdate DESC NULLS LAST
          LIMIT 1
        `
        if (batch.length > 0) {
          await sql`
