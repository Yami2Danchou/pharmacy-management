...
export async function POST(request) {
  try {
    ...
    `
    if (users.length === 0) return NextResponse.json({ error: 'Invalid username or password' }, { status: 401 })
    ...
    return NextResponse.json({ success: true, user: { userId: user.user_id, username: user.username, role: user.role, employeeName: user.employee_name } })
  } catch (err) {
