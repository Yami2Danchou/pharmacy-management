  ...
  try {
    const supplies = await sql`
      SELECT su.supply_id, su.supply_date, su.total_amount,
        sp.supplier_name, u.username,
        COUNT(sd.product_id) as item_count
      FROM supply su
...
export async function POST(request) {
  ...
  try {
    ...
    if (!items || items.length === 0) {
      return NextResponse.json({ error: 'No items in supply' }, { status: 400 })
    }
    ...
    for (const item of items) {
      ...
      // Update or create batch in product_expirationdate
