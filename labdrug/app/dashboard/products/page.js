...
export default function ProductsPage() {
  ...
  const columns = [
    ...
    { key: 'current_stock', label: 'Stock', style: { textAlign: 'right' }, render: r => {
      ...
      const low = stock <= Number(r.reorder_level)
      return <span className={low ? 'text-danger font-medium' : ''}>{stock} {low && stock === 0 ? <span className="badge badge-danger" style={{ marginLeft: 4 }}>Out</span> : low ? <span className="badge badge-warning" style={{ marginLeft: 4 }}>Low</span> : null}</span>
    }},
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 style={{ fontSize: '1.2rem', fontWeight: 700 }}>Products</h2>
          <p className="text-sm text-muted">Manage medicines and pharmacy products</p>
        </div>
      ...
      <div className="card">
        {loading ? (
          ...
          <DataTable
            columns={columns}
            data={products}
            searchKeys={['product_name', 'brand_name', 'category_name']}
            actions={row => (
            ...
            <div className="modal-body">
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input value={form.product_name || ''} onChange={e => setForm({ ...form, product_name: e.target.value })} placeholder="e.g. Paracetamol 500mg" />
                </div>
                <div className="form-group">
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input type="number" min="0" value={form.reorder_level || ''} onChange={e => setForm({ ...form, reorder_level: e.target.value })} placeholder="10" />
                  <p className="form-hint">Alert when stock falls to this level</p>
                </div>
              </div>
              <div className="form-group">
                <label className="form-label">Description</label>
                <textarea rows={2} value={form.description || ''} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Optional product description..." />
              </div>
            </div>
